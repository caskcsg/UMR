...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.feature_map = nn.Linear(640, 256)

        self.img_BayesCap = BayesCap_MLP(inp_dim=256, out_dim=256, hid_dim=64, num_layers=3, p_drop=0.1)
        self.txt_BayesCap = BayesCap_MLP(inp_dim=256, out_dim=256, hid_dim=64, num_layers=3, p_drop=0.1)
        
        self.mlp_encoder = MLPEncoder(activate='gelu', d_in=[100, 2, 256], d_hiddens=[[50, 2, 64], [10,2,32]], d_outs=[[50, 2, 64], [10,2,32]], dropouts=[0.5,0.5,0.5], bias=False, ln_first=False, res_project=[True,True])
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        x = torch.stack([text_embeds.last_hidden_state,image_embeds[:,:100,:]], dim=2)
        x = self.mlp_encoder(x, mask=None)

        # features_mf = torch.bmm(image_features.unsqueeze(2), text_features.unsqueeze(1)) # [batch_size, d, d]
        ...
        loss_i, theta_i = self.Cri(img_mu, img_1alpha, img_beta, image_features, T1=1e0, T2=1e-4)  #改成1e-4损失会变小
        loss_t, theta_t = self.Cri(txt_mu, txt_1alpha, txt_beta, text_features, T1=1e0, T2=1e-4)
        #cross modal terms
        loss_i4t, _ = self.Cri(img_mu, img_1alpha, img_beta, text_features, T1=1e0, T2=1e-4)
        loss_t4i, _ = self.Cri(txt_mu, txt_1alpha, txt_beta, image_features, T1=1e0, T2=1e-4)
        ...
        # print(logits)
        #print(loss_cl)
        output['loss'] = torch.nn.BCEWithLogitsLoss()(logits, torch.unsqueeze(targets.float(), dim=1)) + 0.001 * loss_cl + 0.01 * nce_loss
