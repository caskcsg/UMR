...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        loss_i, theta_i = self.Cri(img_mu, img_1alpha, img_beta, image_features, T1=1e0, T2=1e-5)  
        loss_t, theta_t = self.Cri(txt_mu, txt_1alpha, txt_beta, text_features, T1=1e0, T2=1e-5)
        #cross modal terms
        loss_i4t, _ = self.Cri(img_mu, img_1alpha, img_beta, text_features, T1=1e0, T2=1e-5)
        loss_t4i, _ = self.Cri(txt_mu, txt_1alpha, txt_beta, image_features, T1=1e0, T2=1e-5)
