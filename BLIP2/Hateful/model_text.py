...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.cls_head = nn.Sequential(
                  nn.Linear(1536, 1536,dtype=torch.bfloat16), #1536
                  nn.ReLU(),
                  nn.Linear(1536, 1,dtype=torch.bfloat16)#ve是三分类需要改成二分类
                )

    def forward(self, image, text, targets, alpha=0, train=True):
        image_embeds = self.blip2.get_qformer_features(image).pooler_output  #1408
        text_embeds = self.blip2.get_text_features(text).pooler_output #768
