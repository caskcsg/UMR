...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.blip2 = Blip2Model.from_pretrained("/mnt/raid/yangcp/checkpoint/BLIP2/opt", torch_dtype=torch.bfloat16)
        ...
        # self.image_map = nn.Linear(768, 64 ,dtype=torch.bfloat16)
        self.text_map = nn.Linear(50272, 768,dtype=torch.bfloat16)

        self.cls_head = nn.Sequential(
                  nn.Linear(1536, 768,dtype=torch.bfloat16), #1536
                  nn.ReLU(),
                  nn.Linear(768, 1,dtype=torch.bfloat16)#ve是三分类需要改成二分类
                )

    def forward(self, image, text, targets, alpha=0, train=True):
        image_embeds = self.blip2.get_qformer_features(image).pooler_output  #1408
        text_embeds = self.blip2.get_text_features(text.input_ids, attention_mask = text.attention_mask, return_dict = True) #768
        text_embeds = self.text_map(text_embeds[0][:,0,:])
