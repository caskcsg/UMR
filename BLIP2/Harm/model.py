...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.blip2 = Blip2Model.from_pretrained("/home/luwang/ycp/HM/blip2-flan-t5-xl", device_map='auto')
        ...
        self.image_map = nn.Linear(1408, 256)
        self.text_map = nn.Linear(2048, 256)
        self.feature_map = nn.Linear(320, 256)    

        # self.img_BayesCap_proj = nn.Linear(1408, 768,dtype=torch.bfloat16)
        self.img_BayesCap = BayesCap_MLP(inp_dim=256, out_dim=256, hid_dim=128, num_layers=3, p_drop=0.1)
        self.txt_BayesCap = BayesCap_MLP(inp_dim=256, out_dim=256, hid_dim=128, num_layers=3, p_drop=0.1)
        ...
        self.cls_head = nn.Sequential(
                  nn.Linear(2304, 1252),
                  nn.ReLU(),
                  nn.Linear(1252, 1)#ve是三分类需要改成二分类
                )

    def forward(self, image, text, targets, targets_inputs, alpha=0, train=True):
        image_embeds = self.blip2.get_image_features(image)  #1408
        text_embeds = self.blip2.get_text_features(input_ids=text.input_ids, decoder_input_ids=text.input_ids, return_dict=True) #768
        # print(text_embeds)
        # print(text_embeds.encoder_last_hidden_state.shape)
        image_embeds_mp = self.image_map(image_embeds.last_hidden_state) ##1408
        text_embeds_mp = self.text_map(text_embeds.encoder_last_hidden_state) #2048


        # tmp_img = self.img_BayesCap_proj(image_embeds.last_hidden_state[:,0,:])
        image_features = F.normalize(image_embeds_mp[:,0,:], p=2, dim=1)
        text_features = F.normalize(text_embeds_mp[:,0,:], p=2, dim=1)
