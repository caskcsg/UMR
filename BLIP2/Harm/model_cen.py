    ...
    def __init__(
        ...
        for layer in range(num_layers):
            if layer==0:
                incoming = inp_dim
                outgoing = hid_dim
                mod.append(nn.Linear(incoming, outgoing,dtype=torch.bfloat16))
                mod.append(nn.ReLU())
        ...
        self.block_mu = nn.Sequential(
            nn.Linear(out_dim, out_dim,dtype=torch.bfloat16),
            nn.ReLU(),
            nn.Linear(out_dim, out_dim,dtype=torch.bfloat16),
        )
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        if train:
            ...
            loss_i = self.Cri(img_mu.type(torch.float32), img_1alpha.type(torch.float32), img_beta.type(torch.float32), image_features.type(torch.float32), T1=1e0, T2=5e-2)  #改成1e-4损失会变小
            loss_t = self.Cri(txt_mu.type(torch.float32), txt_1alpha.type(torch.float32), txt_beta.type(torch.float32), text_features.type(torch.float32), T1=1e0, T2=5e-2)
            #cross modal terms
            loss_i4t = self.Cri(img_mu.type(torch.float32), img_1alpha.type(torch.float32), img_beta.type(torch.float32),  text_features.type(torch.float32), T1=1e0, T2=5e-2)
            loss_t4i = self.Cri(txt_mu.type(torch.float32), txt_1alpha.type(torch.float32), txt_beta.type(torch.float32),  image_features.type(torch.float32),T1=1e0, T2=5e-2)
            
            loss_cl = loss_i + loss_t + 1e-6*(loss_i4t + loss_t4i)


            evi_alp = self.softplus(logits.type(torch.float32)) + 1.0
            # Calculate loss
            # print(torch.zeros_like(preds_proxy).shape)
            # print(targets.unsqueeze(-1).shape)
            
            labels_1hot = torch.zeros_like(logits.type(torch.float32)).scatter_(0, targets.unsqueeze(-1), 0)
                # IEDL -> fisher_mse
        ...
        # print(logits)
        #print(loss_cl)
        output['loss'] = torch.nn.BCEWithLogitsLoss()(logits, torch.unsqueeze(targets.float(), dim=1)) + 0.00001 * loss_cl + 0.1 * grad_loss  +  0.0001 * nce_loss
