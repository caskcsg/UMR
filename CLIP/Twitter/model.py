...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.clip = CLIPModel.from_pretrained("/home/yangcp/ALBEF/clip_large")
        ...
        self.image_map = nn.Linear(1024, 64)
        ...
        self.cls_head = nn.Sequential(
                  nn.Linear(4096, 2048),
                  nn.ReLU(),
                  nn.Linear(2048, 1)#ve是三分类需要改成二分类
                )
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        text_embeds = self.text_encoder(text.input_ids, attention_mask = text.attention_mask, return_dict = True).pooler_output
        ...
        if train:
            ...
            loss_i = self.Cri(img_mu, img_1alpha, img_beta, image_features, T1=1e0, T2=5e-2)  #改成1e-4损失会变小
            loss_t = self.Cri(txt_mu, txt_1alpha, txt_beta, text_features, T1=1e0, T2=5e-2)
            #cross modal terms
            loss_i4t = self.Cri(img_mu, img_1alpha, img_beta, text_features, T1=1e0, T2=5e-2)
            loss_t4i = self.Cri(txt_mu, txt_1alpha, txt_beta, image_features, T1=1e0, T2=5e-2)
