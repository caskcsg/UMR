...
from data.hateful_dataset import hateful_dataset
from data.twitter_dataset import twitter_dataset
from data.multioff_dataset import multioff_dataset
from data.harmC_dataset import harmC_dataset
from data.harmP_dataset import harmP_dataset
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        train_dataset = hateful_dataset(config['train_file'], transform_train, config['image_root'])  
        val_seen_dataset = hateful_dataset(config['val_seen_file'], transform_test, config['image_root'])  
        test_seen_dataset = hateful_dataset(config['test_seen_file'], transform_test, config['image_root'])     
        return train_dataset, val_seen_dataset, test_seen_dataset

    elif dataset=='harmP':   
        train_dataset = harmP_dataset(config['train_file'], transform_train, config['image_root'])  
        val_seen_dataset = harmP_dataset(config['val_seen_file'], transform_test, config['image_root'])  
        test_seen_dataset = harmP_dataset(config['test_seen_file'], transform_test, config['image_root'])     
        return train_dataset, val_seen_dataset, test_seen_dataset
    
    elif dataset=='harmC':   
        train_dataset = harmC_dataset(config['train_file'], transform_train, config['image_root'])  
        val_seen_dataset = harmC_dataset(config['val_seen_file'], transform_test, config['image_root'])  
        test_seen_dataset = harmC_dataset(config['test_seen_file'], transform_test, config['image_root'])     
        return train_dataset, val_seen_dataset, test_seen_dataset
        
    elif dataset=='twitter':   
        train_dataset = twitter_dataset(config['train_file_t'], transform_train, config['image_root_t'])  
        val_dataset = twitter_dataset(config['val_file_t'], transform_test, config['image_root_t'])  
        test_dataset = twitter_dataset(config['test_file_t'], transform_test, config['image_root_t']) 
        return train_dataset, val_dataset, test_dataset  

    elif dataset=='multioff':   
        train_dataset = multioff_dataset(config['train_file_m'], transform_train, config['image_root_m'])  
        val_dataset = multioff_dataset(config['val_file_m'], transform_test, config['image_root_m'])  
        test_dataset = multioff_dataset(config['test_file_m'], transform_test, config['image_root_m']) 
        return train_dataset, val_dataset, test_dataset  
