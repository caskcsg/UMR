...
class GenGaussLoss(nn.Module):
	...
	def forward(
		...
		# resi = torch.where(torch.isnan(resi), torch.tensor(0.0).to(beta1.device), resi)
		...
		#torch.distributions.gamma.Gamma(one_over_alpha1, beta1)
		lgamma_up = torch.lgamma(3*torch.pow(beta1, -1))
		lgamma_down = torch.lgamma(torch.pow(beta1, -1))
		theta = one_over_alpha1**2 * torch.exp(lgamma_up / lgamma_down)
...
class TempCombLoss(nn.Module):
	...
	def forward(
		...
		l2, theta = self.L_GenGauss(mean, one_over_alpha, beta, target)
		l = T1*l1 + T2*l2
